...
const PosPage: React.FC<PosPageProps> = ({ 
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full bg-slate-50">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <div className="p-6 bg-white border-b border-slate-100 shadow-sm z-10">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
              <h1 className="text-2xl font-bold text-slate-800">Menu Pesanan</h1>
            ...
            <div className="relative w-full sm:w-72">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-100 border-none rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none text-slate-700"
                value={searchQuery}
              ...
              {filteredProducts.map(product => {
                ...
                return (
                  <div 
                    ...
                    className={`group bg-white rounded-2xl border shadow-sm transition-all overflow-hidden flex flex-col relative
                      ${isOutOfStock ? 'opacity-60 grayscale border-slate-200 cursor-not-allowed' : 'border-slate-100 hover:shadow-md hover:border-blue-200 cursor-pointer'}
                    `}
                  >
                    {isOutOfStock && (
                      <div className="absolute inset-0 z-10 bg-white/50 flex items-center justify-center">
                        <span className="bg-red-600 text-white px-3 py-1 rounded-full text-xs font-bold shadow-sm">STOK HABIS</span>
                      </div>
                    )}
                    ...
                    <div className="h-36 bg-slate-100 relative overflow-hidden">
                      ...
                      />
                      <div className="absolute top-2 right-2 bg-white/90 backdrop-blur-sm px-2 py-1 rounded-lg text-xs font-semibold shadow-sm text-slate-700 flex items-center gap-1">
                         {product.isUnlimited ? (
                    ...
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <p className="text-sm text-slate-500 mb-3 line-clamp-2 flex-1">{product.description || 'Menu spesial cafe.'}</p>
                      <div className="font-bold text-blue-600">{formatRupiah(product.price)}</div>
                    </div>
                  </div>
                );
              })}
      ...
      {/* Right: Cart / Sidebar */}
      <div className="w-full lg:w-96 bg-white border-l border-slate-200 flex flex-col h-[50vh] lg:h-full shadow-xl lg:shadow-none z-20">
        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <h2 className="font-bold text-lg text-slate-800">Pesanan Saat Ini</h2>
          <button 
            onClick={onClearCart}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-400 p-8 text-center">
              ...
              <p className="font-medium text-slate-500 mb-1">Keranjang Kosong</p>
              <p className="text-sm">Silakan pilih menu dari daftar untuk membuat pesanan.</p>
            </div>
          ) : (
            cart.map(item => (
              <div key={item.id} className="flex gap-3 p-3 bg-white rounded-xl border border-slate-100 shadow-sm">
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="font-bold text-slate-800 text-sm">{formatRupiah(item.price * item.quantity)}</span>
                  </div>
                  <div className="flex justify-between items-end mt-2">
                    <span className="text-xs text-slate-400">@ {formatRupiah(item.price)}</span>
                    <div className="flex items-center gap-3 bg-slate-50 rounded-lg p-1 border border-slate-100">
                      <button 
                        onClick={(e) => { e.stopPropagation(); onUpdateCart(item.id, -1); }}
                        className="w-6 h-6 flex items-center justify-center bg-white rounded shadow-sm hover:text-blue-600 hover:border-blue-200 border border-transparent transition-all"
                      >
                      ...
                      <button 
                        ...
                        className={`w-6 h-6 flex items-center justify-center bg-white rounded shadow-sm border border-transparent transition-all
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-200">
           {/* Customer Name Input */}
           <div className="mb-4">
              <label className="block text-xs font-semibold text-slate-500 mb-1 ml-1">Nama Pelanggan (Opsional)</label>
          ...
          <button 
            ...
            disabled={cart.length === 0}
            className="w-full py-4 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white rounded-xl font-bold shadow-lg shadow-blue-200 transition-all transform active:scale-[0.98] flex justify-center items-center gap-2"
          >
             Proses Pembayaran
          </button>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
              <div>
                ...
                <p className="text-slate-500 text-sm mt-1">Total Tagihan: <span className="font-bold text-blue-600">{formatRupiah(total)}</span></p>
              </div>
            ...
            <div className="p-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-5 gap-2">
                        ...
                        <button 
                            ...
                            className="px-1 py-3 text-xs font-bold bg-white hover:bg-slate-50 border border-slate-200 hover:border-blue-300 text-slate-700 rounded-lg transition-all active:scale-95"
                        >
                            +{amount / 1000}k
                        </button>
                        ))}
              ...
              {checkoutStep === 'success' && (
                <div className="text-center py-4">
                  ...
                  <h4 className="text-2xl font-bold text-slate-800 mb-2">Pembayaran Berhasil!</h4>
                  <p className="text-slate-500 mb-6">Transaksi telah tersimpan dalam sistem.</p>
